...
def run_model(opt, train, val, ixtoword):
    ...
    with tf.Session(config = config) as sess:
        ...
        # np.savetxt('./text_arxiv/vae_words.txt', valid_text, fmt='%i', delimiter=' ')
        # pdb.set_trace()
        ...
        for epoch in range(opt.max_epochs):
            ...
            for _, train_index in kf:
                ...
                if uidx % opt.valid_freq == 0:
                    ...
                    np.savetxt('./text_arxiv/syn_val_words.txt',valid_text, fmt='%i', delimiter=' ')
                ...
                if uidx % opt.print_freq == 0:
                    ...
                    np.savetxt('./text_arxiv/syn_train_words.txt', res['syn_sent'], fmt='%i', delimiter=' ')
                    print("Sent:" + u' '.join([ixtoword[x] for x in res['syn_sent'][0] if x != 0]).encode('utf-8').strip())
            ...
            # saver.save(sess, './save_arxiv/arxiv.ckpt')
